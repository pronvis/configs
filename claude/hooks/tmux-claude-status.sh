...
#   Notification      waiting   Claude needs the user (permission, or idle
#                               with a question outstanding)
#   Stop              idle      the turn ended
#   SessionEnd        gone      deregister
#
# Deliberately dependency-free and does no JSON parsing: it runs on every tool
# call, so it stays a couple of syscalls. The pane id is enough to identify the
# session, because `claude-sessions` resolves names from the live tmux pane list.
...
# Drain the hook payload on stdin so Claude Code never writes into a closed
# pipe. We need nothing from it, but leaving it unread risks EPIPE upstream.
[ -t 0 ] || cat >/dev/null
